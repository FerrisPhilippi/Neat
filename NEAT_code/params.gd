...
func load_config(config_name: String) -> void:
	...
	"""
	var dir = DirAccess.open("res://addons/Neat/param_configs")
	var config = ConfigFile.new()
	# If no param configs have been saved yet, save the settings from this file as Default
	if dir == null:
		DirAccess.open("res://addons/Neat/").make_dir("param_configs")
		dir = DirAccess.open("res://addons/Neat/param_configs")
		save_config("Default")
	# try to open the specified file, break execution if it doesn't exist
	else:
		var err = config.load("res://addons/Neat/param_configs/%s.cfg" % config_name)
		if err == OK:
...
func save_config(config_name: String) -> void:
	...
	config.save("res://addons/Neat/param_configs/%s.cfg" % config_name)
